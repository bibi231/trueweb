...
import { Send, Star, CreditCard, X } from "lucide-react";
...
export default function MessagesPage() {
  ...
  const send = async () => {
    ...
    setSending(true);
    setInput("");
    setMsgs((m) => [...m, { id: Date.now(), senderRole: "client", content: text, type: "text", createdAt: new Date().toISOString() }]);
    try {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100svh - 64px)" }}>
      ...
      <div style={{ display: "flex", gap: 10, paddingTop: 14, borderTop: "1px solid var(--border)" }}>
        ...
        <button onClick={send} disabled={!input.trim() || sending} className="btn-teal" style={{ padding: "10px 16px", borderRadius: 10 }}><Send size={15} /></button>
      </div>
    </div>
  );
}
