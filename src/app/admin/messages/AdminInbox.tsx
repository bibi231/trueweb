...
import { Send, CreditCard, Star, X } from "lucide-react";
...
type Msg = {
  ...
  meta?: { amount?: number; url?: string; ref?: string } | null;
  createdAt?: string;
};

const naira = (kobo: number) => `₦${(kobo / 100).toLocaleString("en-NG")}`;
const timeOf = (iso?: string | null) => (iso ? new Date(iso).toLocaleString("en-NG", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }) : "");
...
export function AdminInbox() {
  ...
  const send = async () => {
    ...
    setInput("");
    setMsgs((m) => [...m, { id: Date.now(), senderRole: "admin", content: text, type: "text", createdAt: new Date().toISOString() }]);
    await fetch("/api/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ conversationId: active.id, content: text }) });
    loadMsgs(active.id);
    loadConvs();
  };
  ...
  const sendPayment = async () => {
    if (!active) return;
    const naira = parseFloat(payAmount);
    if (!naira || naira <= 0 || payDesc.trim().length < 2) return;
    ...
    try {
      ...
      await fetch("/api/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ conversationId: active.id, type: "payment_link", content: payDesc.trim(), meta: { amount: kobo, url: data.checkoutUrl, ref: data.reference } }) });
  ...
  const askReview = async () => {
    if (!active) return;
    await fetch("/api/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ conversationId: active.id, content: "We'd love your feedback — could you leave us a quick review from the button above your messages? It really helps." }) });
    loadMsgs(active.id);
  };
        ...
        {!active ? (
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 10, padding: "12px 18px", borderTop: "1px solid var(--border)" }}>
              <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && send()} placeholder={`Reply to ${active.clientName || "client"}…`}
              ...
              <button onClick={send} disabled={!input.trim()} className="btn-teal" style={{ padding: "10px 16px", borderRadius: 10 }}><Send size={15} /></button>
            </div>
          </>
        )}
      </div>
